...
- `prompt`
  ...
  ```javascript
  result = prompt(title, [default]); //  `default` 周围的方括号表示该参数是可选的，不是必需的。
  ```
...
**字符串转换** —— 转换发生在输出内容的时候，也可以通过 `String(value)` 进行显式转换。原始类型值的 string 类型转换通常是很明显的（null => 'null' 等）。
...
当对**不同类型的值**进行比较时，JavaScript 会首先将其**转化为数字（number）**再判定大小。
...
JavaScript 中有四个逻辑运算符：`||`（或），`&&`（与），`!`（非），`??`（空值合并运算符）。虽然它们被称为“逻辑”运算符，但这些运算符却可以被应用于任意类型的值，而不仅仅是布尔值。它们的结果也同样可以是任意类型。
...
+ 理解加载瀑布图：横向+纵向（看是否可以同步发起）

  ![image-20260730135930396](assets/image-20260730135930396.png)

+ 理解HAR存储与重建性能信息：如果短时间内不能分析完所有的内容或者想把该请求导出给其他人协作分析，则可以右键选择【Save all as HAR with content】

+ 帧率FPS：使用【ctrl + shift + P】快捷键，输入【frame】，选择【Show frames per second（FPS） meter】可以查看交互的帧数的变化，看交互是否顺滑

  ![image-20260730140437931](assets/image-20260730140437931.png)

+ 重要测量指标：
  + 速度指数（Speed Index值为4左右就表示优秀了，越小越好）
  + TTFB（请求发出到响应回来的时间：后台的处理速度）
  + 页面加载时间
  + 首次渲染

+ RAIL评估标准:
  + Response 响应：网站给用户的响应的体验，处理事件应在50ms内完成
  + Animation 动画：动画的流畅程度，每10ms产生一帧
  + Idle 空闲：不能让主线程始终处于一个繁忙的状态，这样他就没有足够的时间来处理用户的交互，尽可能增加空闲时间
  + Loading 加载：指的是资源的网络加载时间，在5s内完成内容加载并可以交互（很难实现）
+ 性能测试工具：
  + Chrome DevTools开发调试、性能测试
  + Lighthouse 网站整体质量评估
  + WebPageTest 多测试地点、全能性能报告（https://www.webpagetest.org/）
